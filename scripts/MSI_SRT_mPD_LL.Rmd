...
These directories should have already been created if you already ran the main.Rmd script. Run the following chunk of code in case you didn't. Modify the project.dir and working.dir paths depending on the folder where you are located when running this script. You can check the location where you are using running getwd() directly in the console and modify it using setwd() function. When writing this script we are working in the /sma/scripts folder (our working.dir), and we counsidered sma our project.dir.
```{r message=FALSE, warning=FALSE}
...
It looks like the MSI section has some data outside of the tissue. We will get rid of it using a strategy that involves looking at the first components of the PCA.
...
Based on these selected components, I think we can filter our MSI dataset 
...
Run the alignment function ofn each dataset.
...
The alignment seems to have gone pretty well :-)
