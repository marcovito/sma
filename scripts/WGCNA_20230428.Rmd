...
setwd("/home/marco.vicari/projects/test")
```
...
# LOAD DATA LIST
project.dir <- "/home/marco.vicari/projects/sma_final_sub_230729/sma" #change this and the following paths depending on where you placed the data folder
...
if(file.exists("peak_associated_genes.xlsx")){ file.remove("peak_associated_genes.xlsx")}
if(file.exists("peak_modules.xlsx")){ file.remove("peak_modules.xlsx")}
wb3 <- openxlsx::createWorkbook("peak_associated_genes.xlsx")
wb4 <- openxlsx::createWorkbook("peak_modules.xlsx")
...
for(i in names(analysis_list)){ 
  ...
  saveRDS(joint_PC, paste0(i,"_joint_PCA_ST_MSI.rds"))
  ...
  saveRDS(knn, paste0(i,"_MSI_ST_joint_knn.rds"))
}


openxlsx::saveWorkbook(wb3, "peak_associated_genes.xlsx", overwrite = TRUE)
openxlsx::saveWorkbook(wb4, "peak_modules.xlsx", overwrite = TRUE)
```
...
# CREATE NEW OPTPUT EXCEL FILES/WORKBOOKS
if(file.exists("Joint_SRT_MSI_spinglass_modules.xlsx")){ file.remove("Joint_SRT_MSI_spinglass_modules.xlsx")}
wb <- openxlsx::createWorkbook("Joint_SRT_MSI_spinglass_modules.xlsx")

# FOR EACH MSI MATRIX, DO:
for(i in names(analysis_list)){
  
  # LOAD THE JOINT GRAPHDS
  knn <- readRDS( paste0(i,"_MSI_ST_joint_knn.rds"))
  ...
  write.csv( 
    ...
    file = paste0(i,"spinglass_modules.csv"),row.names = T)
  
  
  # PLOT JOINT GRAPH
  message("   > plotting graph")
  pdf(paste0(i,"_ST_MSI_KNN_layout.pdf"), width=23, heigh=10, useDingbats = F)
    # (PER OMICS)
...
openxlsx::saveWorkbook(wb, "Joint_SRT_MSI_spinglass_modules.xlsx", overwrite = TRUE)
```
...
pdf(paste0("MSI_peack_graph.pdf"),width = 20,height = 5,useDingbats = F)
...
for(i in names(analysis_list) ){
  ...
  # SAVE JOINT GRAPH
  saveRDS(knn, paste0(i,"_MSI_MSI_joint_knn.rds"))
  saveRDS(nn, paste0(i,"_MSI_MSI_joint_knn_matrix.rds"))
  saveRDS(g, paste0(i,"_MSI_MSI_joint_knn_igraph.rds"))
  ...
  knn3 <- matrix(rownames(ST2_use)[c(knn2$idx) ],ncol = 10,byrow = F,dimnames = list(rownames(MSI_data)))
  ...
  # SAVE INTEGRATED GRAPH AND MATRIX
  saveRDS(knn2, paste0(i,"_MSI_ST_joint_knn.rds"))
  saveRDS(nn, paste0(i,"_MSI_ST_joint_knn_matrix.rds"))
